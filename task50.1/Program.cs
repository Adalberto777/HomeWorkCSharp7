...
//(если судить о задаче исходя из условия требуется найти число по его индексам)
...
{
   if(x1 > array.GetLength(0) - 1 || y1 > array.GetLength(1) - 1 || x1 < 0 || y1 <0) // || когда проверить что бы когда хотя бы одно условие выполняется 
   {
